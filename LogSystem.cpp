...
LogRecord::LogRecord()
    : _dateTime(), _temperature(0), _humidity(0)
{    
}
...
LogRecord::LogRecord(const DateTime &dateTime, int8_t temperature, uint8_t humidity)
...
{
    return _dateTime.unixtime() == 0 && _humidity == 0 && _temperature == 0;
}


namespace {
    const char WRITE_FORMAT[] PROGMEM = "%04d-%02d-%02d %02d:%02d:%02d,%d,%d";
    const int WRITE_STR_MAXLEN = 64;
}


void LogRecord::writeToSerial() const
{
    char line[WRITE_STR_MAXLEN];
    sprintf_P(line, WRITE_FORMAT, _dateTime.year(), _dateTime.month(), _dateTime.day(), _dateTime.hour(), _dateTime.minute(), _dateTime.second(), _temperature, _humidity);
    Serial.println(line);
}
...
inline InternalLogRecord getInternalRecord(Storage *storage, uint32_t offset, uint32_t index)
{
    ...
    for (uint8_t i = 0; i < sizeof(InternalLogRecord); ++i) {
        *recordPtr = storage->readByte(storageIndex);
        ++storageIndex;
        ++recordPtr;
    }
    return record;
...
inline void setInternalRecord(Storage *storage, uint32_t offset, InternalLogRecord *record, uint32_t index)
{
    ...
    uint32_t storageIndex = getRecordStart(offset, index);
    for (uint8_t i = 0; i < sizeof(InternalLogRecord); ++i) {
        storage->writeByte(storageIndex, *recordPtr);
        ++storageIndex;
        ++recordPtr;
    }
}
...
uint8_t getCRCForInternalRecord(InternalLogRecord *record)
{
    ...
    return (crc>>12) ^ ((crc&0x0f00)>>8) ^ ((crc&0x00f0)>>4) ^ (crc&0x000f);
}
